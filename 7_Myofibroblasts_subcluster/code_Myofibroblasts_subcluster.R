...
Myofibroblasts <- CellCycleScoring(Myofibroblasts, s.features=s.genes, g2m.features=g2m.genes, set.ident=FALSE)
Myofibroblasts = SCTransform(Myofibroblasts, method="glmGamPoi", vars.to.regress=c("nCount_RNA", "percent.mito", "S.Score", "G2M.Score"), verbose=FALSE)
...
p = DimPlot(Myofibroblasts, reduction="umap", group.by="Sample", pt.size=1)+
...
######################################################### 
mydata <- FindClusters(Myofibroblasts, resolution=0.1)
...
write.table(markers, "./Myofibroblasts_subcluster_DEG.txt", col.names=T, row.names=T, quote=F, sep="\t")
############## 
...
colors = c("#FCC66D", "#98D9E4")
cell_label = c("Myofibroblasts 1", "Myofibroblasts 2")
#################################################################################
## 
