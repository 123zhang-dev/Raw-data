...
mydata = subset(mydata, cell_type=="Epithelial cells")
data = FindMarkers(mydata, group.by="Type", ident.1="negative", ident.2="positive", logfc.threshold=0, min.pct=0)
write.table(data, "./Epithelial_negative_vs_positive_DEG.txt", col.names=T, row.names=T, quote=F, sep="\t")
...
core <- result$core_enrichment[64]
core_genes <- str_split(core ,'/')[[1]]
p = gseaNb(
    object = KEGG_ges,
    geneSetID = result$ID[64],
    addPval = T,
    pvalX = 0.95,
    pvalY = 0.8,
    newGsea = T,
...
ggsave("GSEA_positive_cellCycle.pdf", p, width=6, height=6)
##########################################################################
# 
library(ggplot2)
